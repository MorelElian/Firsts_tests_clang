...
template <typename T>
double benchmark(int size,int repeat)
{
    Eigen::Matrix<double,Dynamic,Dynamic> mat(size,size),tmp_mul(size,size);
    for(int i = 0 ;i < size; i++)
    {
        for(int j = 0 ; j<size ; j++)
        {
    ...
    for(int l = 0; l< repeat; l++)
    {
        ...
        mat  = Eigen::Matrix<double,Dynamic,Dynamic>::Ones(size,size) + int(0.00005) * (mat+ tmp_mul);
    }
...
double call_template(int size,int repeat)
{
    ...
    double result = benchmark<int>(size,repeat);
    return result;
}
